...
df_ <- df %>% filter(!is.na(LastPeriodAbsIOPChangeMean), subtype != "AACG", MIGsYorN == 'N') %>% 
  mutate(subtype=relevel(factor(subtype), ref="OAG"),
         coarseWashoutType=factor(washout.type, c("None", "Partial", "Pre", "Both")))
...
print(metareg(~ PreOpIOPMean * subtype + coarseWashoutType, x=m))
