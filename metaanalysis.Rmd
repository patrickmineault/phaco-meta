...
## Main analysis: $\ge 12$ month follow up
...
```{r}
df <- read.data()
df_ <- df %>% filter(!is.na(LastPeriodAbsIOPChangeMean), subtype != "acute", MIGsYorN == 'N') %>% 
...
print(metareg(~ LastPeriodEyes * subtype, x=m))
...
df_ <- df %>% filter(!is.na(LastPeriodAbsIOPChangeMean), subtype == "OAG", MIGsYorN == 'N') %>% 
  mutate(subtype=relevel(factor(subtype), ref="OAG"))
m <- metagen(LastPeriodAbsIOPChangeMean, 
             LastPeriodAbsIOPChangeStdDev / sqrt(LastPeriodEyes), 
             study.name, 
...
meta.analysis.with.sensitivity <- function(missingness='zero') {
  ...
  df_ <- df %>% 
    filter(!is.na(LastPeriodAbsIOPChangeMean), subtype != "acute") %>% 
    mutate(subtype=factor(subtype))
...
```{r fig.width=10.5, fig.height=6, dev="cairo_pdf"}
